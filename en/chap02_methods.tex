...
In biophysics, there are plenty of experimental methods for determining structure of biomolecules.
The structure of biological membranes can be resolved using
...
The resolution of such methods is, however, often insufficient 
...
has allowed the simulations to grow in spatial scales and composition complexity bringing relevance for applications in biology. \citep{perspective_cecam_lugano_2018} 
...
\todo{This requires better wording. Actually, short-range repulsion is stronger, too.}
The electrostatic interaction yields the strongest inter-molecular attraction forces. 
...
which is expressed in terms of interaction energy $\epsilon_{ij}$ and the distance of the potential minimum, $\sigma_{ij}$, 
...
\todo{Add a couple of sentences of on different ways of accounting for electronic polarization.}

\todo{Condense this paragraph.}
...
Electronic continuum correction (ECC) is an implicit model for accounting for electronic polarization. 
...
\todo{Rewrite the following till the end. use an explicit form of Coulomb equation with epsilon and scaled charges to show the scaling factor and skip some unnecessarry text below.}
...
\todo{Paragraph Suggested for removal.}
...
\todo{Make the discussion POPS-agnostic -- talk about charged molecules/lipids.}
...
We found that the optimal value \todo{Optimized w.r.t. what? -- to reproduce cation binding and response.} for the scaling factor of partial charges of the neutral molecules from the Lipid14 force field \citep{dickson14}
...
\hfill
  \todo{find the correct source for form-f by Kucerka.} 
